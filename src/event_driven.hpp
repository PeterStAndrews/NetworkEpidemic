...
class EVENT_DRIVEN{
    ...
    //
    //      get_results: returns the results map containing experiment data
    //
private:
...
public:
    ...
    std::map<std::string, double> get_results();
};
...
void EVENT_DRIVEN::teardown(){
    // type: function
    // definition: reports the outcome of the experiment, parameters and
    // and metadata for the run.
    S = 0;
    I = 0;
    R = 0;
    ...
    results["beta"] = beta;
    results["gamma"] = gamma;
    results["S"] = static_cast<double>(S);
    results["I"] = static_cast<double>(I);
    results["R"] = static_cast<double>(R);
    
}

std::map<std::string, double> EVENT_DRIVEN::get_results(){
    // type: function
    // definition: returns the results map containing experiment data.
    return results;
}
